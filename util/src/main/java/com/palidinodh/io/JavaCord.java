...
public class JavaCord {
  ...
  public static void init(Settings _settings) {
    settings = _settings;
    new DiscordApiBuilder().setToken(settings.getDiscordToken()).login().thenAccept(api -> {
      JavaCord.api = api;
      // System.out.println("Invite Url: " + api.createBotInvite());
      sendMessage(settings.isLocal() ? DiscordChannel.LOCAL : DiscordChannel.ANNOUNCEMENTS,
          settings.getName() + " is now online!");
  ...
  public static void sendMessage(DiscordChannel channel, String msg) {
    ...
    try {
      String channelName = settings.getDiscordChannel(channel);
      if (channelName == null) {
        return;
      }
      msg = msg.replaceAll("<.*>", "");
